...
export interface ServerlessImageHandlerProps {
  ...
  readonly apiDomain?: CfnParameter;
  readonly apiCertificateIamId?: CfnParameter;
  readonly demoUIDomain?: CfnParameter;
  readonly demoUICertificateIamId?: CfnParameter; 
}
...
export class ServerlessImageHandler extends Construct {
  constructor(scope: Construct, id: string, props: ServerlessImageHandlerProps) {
    ...
    try {
      ...
      const isChinaRegionCondition = new cdk.CfnCondition(this, 'IsChinaRegionCondition', {
      ...
      isChinaRegionCondition.overrideLogicalId('IsChinaRegionCondition')

      if (props.apiCertificateIamId) {
        const apiCertificateCondition = new cdk.CfnCondition(this, 'ApiCertificateCondition', {
          expression: cdk.Fn.conditionNot(cdk.Fn.conditionEquals(props.apiCertificateIamId.valueAsString, ''))
        })
        apiCertificateCondition.overrideLogicalId('ApiCertificateCondition')
      }

      if (props.demoUICertificateIamId) {
        const demoUICertificateCondition = new cdk.CfnCondition(this, 'DemoUICertificateCondition', {
          expression: cdk.Fn.conditionNot(cdk.Fn.conditionEquals(props.demoUICertificateIamId.valueAsString, ''))
        })
        demoUICertificateCondition.overrideLogicalId('DemoUICertificateCondition')
      }
      ...
      // cfnApiGateway.parameters = cdk.Fn.conditionIf('IsChinaRegionCondition', {
      ...
      // Here may could be tested in constructs!
      ...
      cfnCloudFrontDistribution.distributionConfig = {
        aliases: props.apiDomain ? [props.apiDomain.valueAsString] : undefined,
        viewerCertificate: props.apiCertificateIamId ? cdk.Fn.conditionIf('ApiCertificateCondition', {
          IamCertificateId: props.apiCertificateIamId.valueAsString,
          SslSupportMethod: 'sni-only',
          MinimumProtocolVersion: 'TLSv1.2_2018'
        }, cdk.Aws.NO_VALUE) : undefined,
        origins: [{
          // domainName: `${apiGateway.restApiId}.execute-api.${cdk.Aws.REGION}.amazonaws.com`,
          domainName: `${apiGateway.restApiId}.execute-api.${cdk.Aws.REGION}.${cdk.Aws.URL_SUFFIX}`,
        ...
        defaultCacheBehavior: {
          ...
          viewerProtocolPolicy: props.apiCertificateIamId ?
            cdk.Fn.conditionIf('ApiCertificateCondition', 'redirect-to-https', 'allow-all').toString() :
            'https-only'
        },
      ...
      // DemoDistribution
      // Here may could be used in aws construcst!
      ...
      cfnDemoDistribution.distributionConfig = {
        aliases: props.demoUIDomain ? [props.demoUIDomain.valueAsString] : undefined,
        viewerCertificate: props.demoUICertificateIamId ? cdk.Fn.conditionIf('DemoUICertificateCondition', {
          IamCertificateId: props.demoUICertificateIamId.valueAsString,
          SslSupportMethod: 'sni-only',
          MinimumProtocolVersion: 'TLSv1.2_2018'
        }, cdk.Aws.NO_VALUE) : { cloudFrontDefaultCertificate: true },
        ...
        defaultCacheBehavior: {
          ...
          viewerProtocolPolicy: props.demoUICertificateIamId ?
            cdk.Fn.conditionIf('DemoUICertificateCondition', 'redirect-to-https', 'allow-all').toString() :
            'redirect-to-https'
        },
        ipv6Enabled: cdk.Fn.conditionIf('IsChinaRegionCondition', false, true),
        enabled: true,
        httpVersion: 'http2',
        logging: {
          includeCookies: false,
        ...
        properties: [
          ...
          // { path: 'configItem', value: { apiEndpoint: `https://${cloudFrontWebDistribution.distributionDomainName}` } },
          {
            path: 'configItem', value: {
              apiEndpoint:
                props.apiDomain ?
                  cdk.Fn.conditionIf('IsChinaRegionCondition',
                    cdk.Fn.conditionIf('ApiCertificateCondition',
                      cdk.Fn.sub('https://${ApiDomain}'),
                      cdk.Fn.sub('http://${ApiDomain}')
                    ),
